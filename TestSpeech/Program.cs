...
static string GetSalutes(VoiceInfo vInfo, int anio) 
    => vInfo.Culture.Name switch
{
...
static string ByeSalute(VoiceInfo vInfo, string name) 
    => vInfo.Culture.Name switch
{
    "es-MX" => $"¡Adiós {name}!",
...
static bool PrintMenu(SpeechSynthesizer synthesizer)
{
    ...
    {
        Console.WriteLine(
            $"{i++}.- {GetPersonName(v.VoiceInfo)} [{v.VoiceInfo.Culture.DisplayName}]");
...
struct LanguageRegion
{
    ...
    public static LanguageRegion Create(CultureInfo cultureInfo) 
        => new LanguageRegion
    {
